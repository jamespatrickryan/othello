...
class Index(tkinter.Frame):
    ...
    def initialize(self):
        ...
        self.draw_a_disk(i, j, BLACK)
        self.draw_a_disk(j, i, BLACK)

        self.draw_a_disk(j, j, WHITE)
        self.draw_a_disk(i, i, WHITE)
    ...
    def put(self, x, y, turn, clone):
        ...
        former = self.turn
        self.turn = then(self.board, former)
    ...
    def computer(self):
        clone = list(self.board)
        coordinates = fetch(minimax_searcher(4, score), self.turn, self.board)
        progress(coordinates, self.turn, self.board)
