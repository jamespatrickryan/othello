...
def legal_moves(player, board):
    return [square
            for square in cells()
            if a_legal(square, player, board)]


def any_legal_moves(player, board):
    return any(a_legal(square, player, board)
               for square in cells())
...
def minimax(player, board, alpha, beta, depth, evaluate):
    ...
    if not moves:
        if not any_legal_moves(opponent(player), board):
            return decider(player, board), None
...
def verify(move, player, board):
    return on_the_board(move) and a_legal(move, player, board)


def human(player, board):
    print(illustrate_the(board))
...
def fetch(strategy, player, board):
    ...
    if not on_the_board(move) or not a_legal(move, player, board):
        raise

    return move


def then(board, former):
    foe = opponent(former)

    if any_legal_moves(foe, board):
        return foe
    elif any_legal_moves(former, board):
        return former
    else:
        return None
